...
def euclidiana(avalicao1, avalicao2):
    ...
    for chave in avalicao1:
        if chave in avalicao2:
            potencia1 = pow(avalicao1[chave] - avalicao2[chave],2)
            distancia = distancia + sqrt(potencia1)
    return distancia
...
def computaVizinho(nomeUsuario, usuarios):
    ...
    for usuario in usuarios:
        if usuario != nomeUsuario:
            distancia = euclidiana(usuarios[usuario], usuarios[nomeUsuario])
...
print(euclidiana(usuarios["Veronica"], usuarios["Sam"]))
